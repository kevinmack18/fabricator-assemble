...
var parseMaterials = function () {
	...
	files.forEach(function (file) {
		...
		if (isSubCollection) {
			assembly.materials[parent].items[collection] = assembly.materials[parent].items[collection] || {
				...
				newname: toTitleCase(getName(parent)),
				items: {}
			};
		}
	...
	files.forEach(function (file) {
		...
		var collection = getName(path.normalize(path.dirname(file)).split(path.sep).pop(), true);
		var directoryPaths = path.normalize(path.dirname(file)).split(path.sep);
		var layerType = directoryPaths[2];
		var parent = directoryPaths.slice(-2, -1)[0];
		var isSubCollection = (dirs.indexOf(parent) > -1);
		...
		var isGlobalLayer = (layerType == "global");
		var isApplicationLayer = (layerType == "applications");
		...
		var layerTypeName = function() {
			...
			if (isGlobalLayer) {
				parentName = layerType;
			} else if(isApplicationLayer) {
				parentName = directoryPaths[3];
			} else {
				parentName = false;
			}
		...
		var layerId = (layerTypeName()) ? toLowerCase(getName(layerTypeName())  + '.' + getName(file)) : false;
		...
		if (layerId) {
			Handlebars.registerPartial(layerId, content);
		}
